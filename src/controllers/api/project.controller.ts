...
export const createJob: RequestHandler<never, string, BodyTypes.Job> = (
    ...
    // Make sure jobid is not provided, as it is generated by the server.
    ...
    // Make sure job is valid, by checking that the requested core exists.
    if (!checkJob(job)) {
        res.status(400);
...
/**
 *  Get job from project database.
 *  @param req Request object. Must contain a projectid and jobid.
 *  @param res Response object. Returns 200 if job was found, along with the job object in the response body. 400 if jobid was not provided. 404 if job was not found.
 * */
...
/**
 *  Update job in project database.
 *  @param req Request object. Must contain a projectid and jobid. Must contain a job object in the request body.
 *  @param res Response object. Returns 200 if job was updated. 400 if jobid/projectid was not provided or job failed validation.
 * */
export const updateJob: RequestHandler<ParamTypes.Job, never, BodyTypes.Job> = (
    ...
    // Make sure that the job in the body refers to the same job/project as the request parameters, and that the job refers to an existing core.
...
/**
 *  Delete job from project database.
 *  @param req Request object. Must contain a projectid and jobid.
 *  @param res Response object. Returns 200 if job was deleted. 400 if jobid/projectid was not provided.
 * */
